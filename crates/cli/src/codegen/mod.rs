...
impl Generator {
    ...
    pub fn resolve_identifiers(&self, module: &mut Module) -> Result<Identifiers> {
        match self.ty {
            ...
            CodeGenType::Dynamic => {
                ...
                // User can use `invoke` with a null function name. User
                // plugins also won't have an `eval_bytecode` function to
    ...
    fn generate_main(
        ...
        if let Some(eval_bytecode) = imports.eval_bytecode {
            ...
            // support calling `invoke` with a null function. Older
            // `javy_quickjs_provider_v3`'s do not support being called with a
            // null function. But because creating dynamically linked modules
            // now require a plugin, it is guaranteed that any plugin they link
            // against will support calling `invoke` with a null function.
